	...
	if ($servant->action()->id() === 'read') {
		...
		if (is_array($level2)) {
			...
			foreach ($level2 as $key => $value) {
				...
				// Link HTML
				$link = '<a href="'.$servant->paths()->userAction('read', 'domain', $servant->article()->tree(0), $key).'">'.$servant->format()->articleName($key).'</a>';
				...
				if (is_array($value)) {
					...
					foreach ($value as $key2 => $value2) {
						...
						// Child item HTML
						$link = '<a href="'.$servant->paths()->userAction('read', 'domain', $servant->article()->tree(0), $key, $key2).'">'.$servant->format()->articleName($key2).'</a>';
